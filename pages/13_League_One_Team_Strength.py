...
st.set_page_config(
    page_title="League One Team Strength",
    layout="wide",
...
def load_data():
    ...
    try:
        ...
        if meta_resp.status_code == 200:
            ...
            div_data = divisions.get("England__League One__3", {})
    ...
    if not last_update_str and 'match_date' in df.columns:
        played = df[(df['league'] == "League One")].dropna(subset=['match_result'])
        if not played.empty:
...
filter_header("Teams", all_teams, "l1_pills_teams")
selected_teams = st.sidebar.pills(
    "Teams", options=all_teams, key="ts_l1_pills_teams",
