...
st.set_page_config(
    page_title="Serie A Team Strength",
    layout="wide",
...
def load_data():
    ...
    try:
        ...
        if meta_resp.status_code == 200:
            ...
            div_data = divisions.get("Italy__Serie A__1", {})
    ...
    if not last_update_str and 'match_date' in df.columns:
        played = df[(df['league'] == "Serie A")].dropna(subset=['match_result'])
        if not played.empty:
...
sa_teams_all = df[df['league'] == 'Serie A']
for _, row in sa_teams_all[['home_team_code', 'home_team']].dropna().drop_duplicates().iterrows():
    team_code_to_name[row['home_team_code']] = row['home_team']
for _, row in sa_teams_all[['away_team_code', 'away_team']].dropna().drop_duplicates().iterrows():
...
all_seasons = sorted([s for s in df[df['league'] == 'Serie A']['season'].dropna().unique() if s >= "2014/15"])
...
sa_df_filtered = df[(df['league'] == 'Serie A') & (df['season'] >= s_start) & (df['season'] <= s_end)]
all_teams = sorted([t for t in set(sa_df_filtered['home_team_code'].dropna()).union(set(sa_df_filtered['away_team_code'].dropna())) if t and str(t).strip()])

if 'ts_sa_pills_teams' not in st.session_state or st.session_state.get('ts_sa_prev_all_teams') != all_teams:
    st.session_state.ts_sa_pills_teams = all_teams
    st.session_state.ts_sa_prev_all_teams = all_teams

search_name = st.sidebar.text_input("Search Team", placeholder="Enter team name...", key="ts_sa_search_name")
...
selected_teams = st.sidebar.pills(
    "Teams", options=all_teams, key="ts_sa_pills_teams",
...
with col1:
    ...
    df_unplayed = df[(df['league'] == 'Serie A') & (df['match_result'].isna())]
...
def load_fixtures_model():
    url = "http://198.244.151.163:8000/serie_a_fixtures_model"
    try:
        df_fix = pd.read_parquet(url)
        return df_fix
    except Exception:
        import os
        local_path = os.path.join(os.path.dirname(__file__), "..", "..", "streamlit", "serie_a_fixtures_model.parquet")
        if os.path.exists(local_path):
...
df_hist = df_hist[df_hist['league'] == 'Serie A']
