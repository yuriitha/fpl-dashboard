...
def load_data():
    ...
    try:
        ...
        if meta_resp.status_code == 200:
            ...
            div_data = divisions.get("Spain__LaLiga__1", {})
    ...
    if not last_update_str and 'match_date' in df.columns:
        played = df[(df['league'] == "LaLiga")].dropna(subset=['match_result'])
        if not played.empty:
...
ll_teams_all = df[df['league'] == 'LaLiga']
for _, row in ll_teams_all[['home_team_code', 'home_team']].dropna().drop_duplicates().iterrows():
    team_code_to_name[row['home_team_code']] = row['home_team']
for _, row in ll_teams_all[['away_team_code', 'away_team']].dropna().drop_duplicates().iterrows():
...
ll_df_filtered = df[(df['league'] == 'LaLiga') & (df['season'] >= s_start) & (df['season'] <= s_end)]
all_teams = sorted([t for t in set(ll_df_filtered['home_team_code'].dropna()).union(set(ll_df_filtered['away_team_code'].dropna())) if t and str(t).strip()])

if 'ts_ll_pills_teams' not in st.session_state or st.session_state.get('ts_ll_prev_all_teams') != all_teams:
    st.session_state.ts_ll_pills_teams = all_teams
    st.session_state.ts_ll_prev_all_teams = all_teams

search_name = st.sidebar.text_input("Search Team", placeholder="Enter team name...", key="ts_ll_search_name")
...
filter_header("Teams", all_teams, "ll_pills_teams")
selected_teams = st.sidebar.pills(
    "Teams", options=all_teams, key="ts_ll_pills_teams",
...
with col1:
    ...
    curr_season_df = df[(df['league'] == 'LaLiga') & (df['season'] == current_season)]
