...
st.set_page_config(
    page_title="2. Bundesliga Team Strength",
    layout="wide",
...
def load_data():
    ...
    try:
        ...
        if meta_resp.status_code == 200:
            ...
            div_data = divisions.get("Italy__2. Bundesliga__2", {})
    ...
    if not last_update_str and 'match_date' in df.columns:
        played = df[(df['league'] == "2. Bundesliga")].dropna(subset=['match_result'])
        if not played.empty:
...
b2_teams_all = df[df['league'] == '2. Bundesliga']
for _, row in b2_teams_all[['home_team_code', 'home_team']].dropna().drop_duplicates().iterrows():
    team_code_to_name[row['home_team_code']] = row['home_team']
for _, row in b2_teams_all[['away_team_code', 'away_team']].dropna().drop_duplicates().iterrows():
...
all_seasons = sorted([s for s in df[df['league'] == '2. Bundesliga']['season'].dropna().unique() if s >= "2010/11"])
...
b2_df_filtered = df[(df['league'] == '2. Bundesliga') & (df['season'] >= s_start) & (df['season'] <= s_end)]
all_teams = sorted([t for t in set(b2_df_filtered['home_team_code'].dropna()).union(set(b2_df_filtered['away_team_code'].dropna())) if t and str(t).strip()])

if 'ts_b2_pills_teams' not in st.session_state or st.session_state.get('ts_b2_prev_all_teams') != all_teams:
    st.session_state.ts_b2_pills_teams = all_teams
    st.session_state.ts_b2_prev_all_teams = all_teams

search_name = st.sidebar.text_input("Search Team", placeholder="Enter team name...", key="ts_b2_search_name")
...
filter_header("Teams", all_teams, "sb_pills_teams")
selected_teams = st.sidebar.pills(
    "Teams", options=all_teams, key="ts_b2_pills_teams",
...
with col1:
    ...
    curr_season_df = df[(df['league'] == '2. Bundesliga') & (df['season'] == current_season)]
    current_season_teams = sorted(list(set(curr_season_df['home_team_code'].dropna()).union(set(curr_season_df['away_team_code'].dropna()))))

    df_played = df[(df['league'] == '2. Bundesliga')].dropna(subset=['match_result'])
    ...
    df_unplayed = df[(df['league'] == '2. Bundesliga') & (df['match_result'].isna())]
...
df_hist = df_hist[df_hist['league'] == '2. Bundesliga']
