...
st.set_page_config(
    page_title="Bundesliga Team Strength",
    layout="wide",
...
def apply_team_code_and_color_fallbacks(df):
    ...
    german_defaults = {
        'Bayern Munich': {'abbr': 'BAY', 'color_light': '#DC052D', 'color_dark': '#FF3B56'},
        'Dortmund': {'abbr': 'DOR', 'color_light': '#E0C800', 'color_dark': '#FDE100'},
        'Bayer Leverkusen': {'abbr': 'LEV', 'color_light': '#E32221', 'color_dark': '#FF4D4D'},
        'RB Leipzig': {'abbr': 'RBL', 'color_light': '#DD0741', 'color_dark': '#FF2D65'},
        'Eintracht Frankfurt': {'abbr': 'SGE', 'color_light': '#C0000D', 'color_dark': '#FF2A3A'},
        'VfB Stuttgart': {'abbr': 'VFB', 'color_light': '#E32219', 'color_dark': '#FF473E'},
        'Freiburg': {'abbr': 'FRE', 'color_light': '#B00010', 'color_dark': '#FF3344'},
        'Hoffenheim': {'abbr': 'TSG', 'color_light': '#005CA9', 'color_dark': '#1C86EE'},
        'Union Berlin': {'abbr': 'FCU', 'color_light': '#EB1923', 'color_dark': '#FF4049'},
        'B. Monchengladbach': {'abbr': 'BMG', 'color_light': '#008542', 'color_dark': '#00C864'},
        'Werder Bremen': {'abbr': 'SVW', 'color_light': '#1D9053', 'color_dark': '#2BD67B'},
        'Augsburg': {'abbr': 'AUG', 'color_light': '#BA3733', 'color_dark': '#E04A46'},
        'Mainz': {'abbr': 'M05', 'color_light': '#C3141E', 'color_dark': '#E62E38'},
        '1. FC Koln': {'abbr': 'KOL', 'color_light': '#ED1C24', 'color_dark': '#FF474E'},
        'Hamburger': {'abbr': 'HSV', 'color_light': '#004F9F', 'color_dark': '#0070E0'},
        'Schalke': {'abbr': 'S04', 'color_light': '#004D9D', 'color_dark': '#0070E0'},
        'Paderborn': {'abbr': 'PAD', 'color_light': '#005CA9', 'color_dark': '#1E90FF'},
        'Elversberg': {'abbr': 'ELV', 'color_light': '#C00016', 'color_dark': '#FF334B'},
        'Wolfsburg': {'abbr': 'WOB', 'color_light': '#65B32E', 'color_dark': '#7FE03A'},
        'Heidenheim': {'abbr': 'FCH', 'color_light': '#E30613', 'color_dark': '#FF3340'},
        'St. Pauli': {'abbr': 'STP', 'color_light': '#704214', 'color_dark': '#A0522D'},
        'Bochum': {'abbr': 'BOC', 'color_light': '#005CA9', 'color_dark': '#1E90FF'},
        'Holstein Kiel': {'abbr': 'KSV', 'color_light': '#004F9F', 'color_dark': '#1E90FF'},
        'Darmstadt': {'abbr': 'D98', 'color_light': '#004E9F', 'color_dark': '#1E90FF'},
        'Hertha Berlin': {'abbr': 'BSC', 'color_light': '#004D9D', 'color_dark': '#1E90FF'},
        'Dusseldorf': {'abbr': 'F95', 'color_light': '#E30613', 'color_dark': '#FF3340'},
        'Hannover': {'abbr': 'H96', 'color_light': '#222222', 'color_dark': '#00873D'},
        'Nurnberg': {'abbr': 'FCN', 'color_light': '#8E182A', 'color_dark': '#B52239'},
        'Kaiserslautern': {'abbr': 'FCK', 'color_light': '#E30613', 'color_dark': '#FF3340'},
        'Karlsruher': {'abbr': 'KSC', 'color_light': '#004D9D', 'color_dark': '#1E90FF'},
        'Greuther Furth': {'abbr': 'SGF', 'color_light': '#00873D', 'color_dark': '#00B050'},
        'Magdeburg': {'abbr': 'FCM', 'color_light': '#004D9D', 'color_dark': '#1E90FF'},
        'Braunschweig': {'abbr': 'EBS', 'color_light': '#E6C200', 'color_dark': '#FFD700'},
        'Dresden': {'abbr': 'SGD', 'color_light': '#E6C200', 'color_dark': '#FFD700'},
        'Arminia Bielefeld': {'abbr': 'DSC', 'color_light': '#004D9D', 'color_dark': '#1E90FF'},
        'VfL Osnabruck': {'abbr': 'OSN', 'color_light': '#5C246E', 'color_dark': '#8B3A9E'},
        'Energie Cottbus': {'abbr': 'FCE', 'color_light': '#E30613', 'color_dark': '#FF3340'}
    }
    for k, v in german_defaults.items():
        if k not in team_dict:
            team_dict[k] = v
...
def load_data():
    ...
    try:
        ...
        if meta_resp.status_code == 200:
            ...
            div_data = divisions.get("Italy__Bundesliga__1", {})
    ...
    if not last_update_str and 'match_date' in df.columns:
        played = df[(df['league'] == "Bundesliga")].dropna(subset=['match_result'])
        if not played.empty:
...
b1_teams_all = df[df['league'] == 'Bundesliga']
for _, row in b1_teams_all[['home_team_code', 'home_team']].dropna().drop_duplicates().iterrows():
    team_code_to_name[row['home_team_code']] = row['home_team']
for _, row in b1_teams_all[['away_team_code', 'away_team']].dropna().drop_duplicates().iterrows():
...
all_seasons = sorted([s for s in df[df['league'] == 'Bundesliga']['season'].dropna().unique() if s >= "2010/11"])
...
b1_df_filtered = df[(df['league'] == 'Bundesliga') & (df['season'] >= s_start) & (df['season'] <= s_end)]
all_teams = sorted([t for t in set(b1_df_filtered['home_team_code'].dropna()).union(set(b1_df_filtered['away_team_code'].dropna())) if t and str(t).strip()])

if 'ts_b1_pills_teams' not in st.session_state or st.session_state.get('ts_b1_prev_all_teams') != all_teams:
    st.session_state.ts_b1_pills_teams = all_teams
    st.session_state.ts_b1_prev_all_teams = all_teams

search_name = st.sidebar.text_input("Search Team", placeholder="Enter team name...", key="ts_b1_search_name")
...
filter_header("Teams", all_teams, "sa_pills_teams")
selected_teams = st.sidebar.pills(
    "Teams", options=all_teams, key="ts_b1_pills_teams",
...
with col1:
    ...
    curr_season_df = df[(df['league'] == 'Bundesliga') & (df['season'] == current_season)]
    current_season_teams = sorted(list(set(curr_season_df['home_team_code'].dropna()).union(set(curr_season_df['away_team_code'].dropna()))))

    df_played = df[(df['league'] == 'Bundesliga')].dropna(subset=['match_result'])
    ...
    df_unplayed = df[(df['league'] == 'Bundesliga') & (df['match_result'].isna())]
...
def load_fixtures_model():
    url = "http://198.244.151.163:8000/bundesliga_fixtures_model"
    try:
        df_fix = pd.read_parquet(url)
        return df_fix
    except Exception:
        import os
        local_path = os.path.join(os.path.dirname(__file__), "..", "..", "streamlit", "bundesliga_fixtures_model.parquet")
        if os.path.exists(local_path):
...
if not df_fixtures.empty:
    ...
    """
    st.html(js_proj_tables, unsafe_allow_javascript=True)
...
df_hist = df_hist[df_hist['league'] == 'Bundesliga']
