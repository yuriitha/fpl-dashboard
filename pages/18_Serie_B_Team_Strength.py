...
st.set_page_config(
    page_title="Serie B Team Strength",
    layout="wide",
...
def load_data():
    ...
    try:
        ...
        if meta_resp.status_code == 200:
            ...
            div_data = divisions.get("Italy__Serie B__2", {})
    ...
    if not last_update_str and 'match_date' in df.columns:
        played = df[(df['league'] == "Serie B")].dropna(subset=['match_result'])
        if not played.empty:
...
sb_teams_all = df[df['league'] == 'Serie B']
for _, row in sb_teams_all[['home_team_code', 'home_team']].dropna().drop_duplicates().iterrows():
    team_code_to_name[row['home_team_code']] = row['home_team']
for _, row in sb_teams_all[['away_team_code', 'away_team']].dropna().drop_duplicates().iterrows():
...
all_seasons = sorted([s for s in df[df['league'] == 'Serie B']['season'].dropna().unique() if s >= "2014/15"])
...
sb_df_filtered = df[(df['league'] == 'Serie B') & (df['season'] >= s_start) & (df['season'] <= s_end)]
all_teams = sorted([t for t in set(sb_df_filtered['home_team_code'].dropna()).union(set(sb_df_filtered['away_team_code'].dropna())) if t and str(t).strip()])

if 'ts_sb_pills_teams' not in st.session_state or st.session_state.get('ts_sb_prev_all_teams') != all_teams:
    st.session_state.ts_sb_pills_teams = all_teams
    st.session_state.ts_sb_prev_all_teams = all_teams

search_name = st.sidebar.text_input("Search Team", placeholder="Enter team name...", key="ts_sb_search_name")
...
selected_teams = st.sidebar.pills(
    "Teams", options=all_teams, key="ts_sb_pills_teams",
...
with col1:
    ...
    df_unplayed = df[(df['league'] == 'Serie B') & (df['match_result'].isna())]
...
df_hist = df_hist[df_hist['league'] == 'Serie B']
